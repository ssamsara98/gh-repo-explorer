...
export const useGitubProfile = ({ username }: { username: string }) => {
  const { data, isLoading, error, refetch } = useGetGithubUserRepositoriesQuery(username);
  ...
  const repos = useMemo(() => {
    return data?.data?.repos.filter((repo) => {
      const searchConditionMeet = repo.full_name.toLowerCase().includes(search.toLocaleLowerCase());

      return searchConditionMeet && filter === 'Unknown'
        ? !repo?.language
        : !filter
        ? true && searchConditionMeet
        : repo?.language?.toLowerCase().includes(filter.toLocaleLowerCase()) && searchConditionMeet;
    });
  }, [data, search, filter]);
